...
local SBC = Singularity --Localise the global table for speed.
SBC.MenuCore = SBC.MenuCore or {}

local MC = SBC.MenuCore
...
if(CLIENT)then
	...
	function MC.CreateFrame(Size,Visible,XButton,Draggable,CloseDelete)
		local Derma = vgui.Create( "DFrame" )
			if Derma then
				...
				function Derma:SetColor(Col) self.RenderColor = Col end Derma:SetColor(SBC.GuiThemeColor.BG) --Dont Forget to set a color so we dont get nil Errors.
				function Derma:SetGradientColor(Col) self.RenderGradientColor = Col end Derma:SetGradientColor(SBC.GuiThemeColor.GC) --Dont Forget to set a color so we dont get nil Errors.
				...
				Derma.Paint = function( self, w, h )
					...
					if GC.a>0 then
						surface.SetTexture( SBC.GradientTex )
					...
					draw.DrawText(self.Title, "DermaDefault",tw+20,th, SBC.GuiThemeColor.Text, 2)
				end		
			end
		return Derma
	end
	...
	function MC.CreatePSheet(Parent,Size,Spot)
		local Derma = vgui.Create( "DPropertySheet", Parent )
			...
			function Derma:AddSheet(label, panel, material, NoStretchX, NoStretchY, Tooltip)
				...
				function Tab:SetColor(Col) self.RenderColor = Col end Tab:SetColor(SBC.GuiThemeColor.FG)
			...
			function Derma:SetColor(Col) self.RenderColor = Col end Derma:SetColor(SBC.GuiThemeColor.FG)
	...
	function MC.CreatePBar(Parent,Size,Spot,Progress)
		local Derma = vgui.Create( "DPanel", Parent )
			...
			function Derma:SetBGColor(Col) self.RenderBGColor = Col end Derma:SetBGColor(SBC.GuiThemeColor.BG) --Dont Forget to set a color so we dont get nil Errors.
	...
	function MC.CreateText(Parent,Spot,Text,Color, Font)
		local Derma = vgui.Create( "DLabel", Parent )
			Derma:SetPos( Spot.x, Spot.y )
			Derma:SetText( Text or "" )
			if Font~= nil then Derma:SetFont(Font) end
			Derma:SetTextColor( Color or SBC.GuiThemeColor.Text )
	...
	function MC.CreateAdvText(Parent,Size,Spot,Text,Color)	
		local Derma = vgui.Create( "DPanel", Parent )
			...
			function Derma:SetTextColor(Col) self.TextColor = Col end Derma:SetTextColor( Color or SBC.GuiThemeColor.Text )
	...
	function MC.CreateButton(Parent,Size,Spot,Text,OnClick)
		local Derma = vgui.Create( "DButton", Parent )
			...
			Derma:SetColor(SBC.GuiThemeColor.Text)
			
			function Derma:SetBGColor(Col) self.RenderColor = Col end Derma:SetBGColor(SBC.GuiThemeColor.FG)
			function Derma:SetGradientColor(Col) self.RenderGradientColor = Col end Derma:SetGradientColor(SBC.GuiThemeColor.GC)
			function Derma:SetGradientColorOver(Col) self.RenderGradientColorOver = Col end Derma:SetGradientColorOver(SBC.GuiThemeColor.GHO)
